"""Train one XGBoost classifier per outcome horizon (Within1Yr ... Within7Yr).

Mirrors Random_Forest/src/train_rf.py exactly: the same nested CV protocol,
the same stability-selection feature step, the same output schema. Only the
classifier (and its hyperparameter grid) is different.

Class imbalance is handled with `scale_pos_weight = #neg / #pos` per fold.
Trees are scale-invariant so the final XGBoost is fitted on the *raw*
selected columns; the StandardScaler is used only inside stability
selection.

Run:
    python XGBoost/src/train_xgb.py
"""
...
def make_xgb(*, scale_pos_weight: float, random_state: int, **params) -> XGBClassifier:
...
def fit_one_outer_fold(
    ...
    """Run stability selection + inner-CV-tuned XGBoost on one outer fold."""
    ...
    # Trees handle raw scales fine; the standardised matrix is only used for L1 selection.
    ...
    base_xgb = make_xgb(scale_pos_weight=spw, random_state=fold_seed)
    gs = GridSearchCV(
        base_xgb, XGB_PARAM_GRID, scoring="roc_auc", cv=inner,
        n_jobs=N_JOBS, refit=True, error_score=np.nan,
    )
    gs.fit(Xtr_sel, y_train)

    best_xgb = gs.best_estimator_
    y_prob = best_xgb.predict_proba(Xte_sel)[:, 1]

    train_prob = best_xgb.predict_proba(Xtr_sel)[:, 1]
...
def train_outcome(df: pd.DataFrame, outcome: str, feature_names: list[str]) -> dict:
    ...
    # ----- final model: stability-select on all data, refit XGB with best CV params -----
    ...
    spw_full = (n_neg / max(n_pos, 1)) if n_pos > 0 else 1.0
    final_xgb = make_xgb(
        scale_pos_weight=spw_full, random_state=RANDOM_STATE, **most_common,
    )
    final_xgb.set_params(n_jobs=N_JOBS)
    final_xgb.fit(X[:, sel_idx_full], y)
    ...
    plot_roc_curves(per_fold_probs, y_true_pool, y_prob_pool,
                    ...
                    f"ROC — {outcome} (XGBoost, nested CV)")
    ...
    plot_top_features(final_features_df["feature"].tolist(),
                      final_features_df["xgb_importance"].tolist(),
                      plots_dir / "xgb_feature_importance.png",
                      f"Top XGBoost feature importance — {outcome}",
