"""XGBoost gradual-training sweep over the published feature ranking.

Mirrors Random_Forest/src_gradual_training/gradual_train.py one-for-one;
only the classifier (and its grid) differs. Reads the per-outcome ranking
from XGBoost/results/Within{k}Yr/final_selected_features.csv (sorted by
`xgb_importance`).

Run:
    python XGBoost/src_gradual_training/gradual_train.py
    python XGBoost/src_gradual_training/gradual_train.py --outcomes Within3Yr
"""
...
def load_ranked_features(outcome: str) -> pd.DataFrame:
    ...
    if not path.exists():
        raise FileNotFoundError(
            f"Cannot find '{path}'. Run XGBoost/src/train_xgb.py first."
        )
    df = pd.read_csv(path).sort_values(RANKING_COLUMN, ascending=False).reset_index(drop=True)
    return df
...
def fit_one_k(
    ...
    for fold_idx, (tr, te) in enumerate(outer.split(X_k, y)):
        ...
        if HYPERPARAM_STRATEGY == "per_fold_grid":
            ...
            base = make_xgb(scale_pos_weight=spw, random_state=fold_seed)
            gs = GridSearchCV(
                base, XGB_PARAM_GRID, scoring=INNER_SCORING, cv=inner,
            ...
            model = make_xgb(
                scale_pos_weight=spw, random_state=fold_seed, **fixed_best_params,
            )
            model.fit(Xtr, ytr)
...
def run_outcome(outcome: str, df: pd.DataFrame) -> dict:
    ...
    peak_summary = {
        ...
        "param_grid": XGB_PARAM_GRID if HYPERPARAM_STRATEGY == "per_fold_grid" else None,
...
def write_outcome_report(out_dir, outcome, metrics_df, peak, ranked,
    ...
    lines = [
        ...
        f"- Feature ranking source: `{MAIN_RESULTS_DIR.name}/{outcome}/"
...
def write_master_report(summary_rows, root_report_path):
    ...
    lines = [
        ...
        f"**{MODEL_FAMILY}** as the classifier. All other components — stability "
        f"selection, nested CV, metrics — are kept identical so that cross-model "
        f"comparison is fair.",
        "",
    ...
    lines += [
        "",
        "Per-outcome detail is in each `Within{k}Yr_results/report.md`.",
        "",
    ]
