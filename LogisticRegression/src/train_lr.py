"""Train one Logistic-Regression classifier per outcome horizon.
...
StandardScaler -> LogisticRegression Pipeline) is different. Unlike
Random Forest / XGBoost, logistic regression is *not* scale-invariant — so
we standardise the selected features inside a Pipeline before fitting LR.

Run:
    python LogisticRegression/src/train_lr.py
"""
...
def lr_importance_from_pipeline(pipe: Pipeline) -> np.ndarray:
    """Importance for an already-scaled LR = absolute coefficient on the
    standardised feature scale (which is what Pipeline yields)."""
...
def fit_one_outer_fold(X_train, y_train, X_test, y_test, feature_names, fold_seed):
    ...
    pipe = make_lr_pipeline(random_state=fold_seed)
    gs = GridSearchCV(
        pipe, LR_PARAM_GRID, scoring="roc_auc", cv=inner,
    ...
    return {
        ...
        "val_brier": val_brier,
        "feature_importances": lr_importance_from_pipeline(best),
        "scaler": scaler,
    }
...
def train_outcome(df: pd.DataFrame, outcome: str, feature_names: list[str]) -> dict:
    ...
    final_pipe = make_lr_pipeline(random_state=RANDOM_STATE)
    final_pipe.set_params(**most_common)
    final_pipe.fit(X[:, sel_idx_full], y)

    final_features_df = pd.DataFrame({
        "feature": sel_names_full,
        "stability_frequency": sel_freq_full[sel_idx_full],
        "lr_importance": lr_importance_from_pipeline(final_pipe),
    ...
    plot_roc_curves(per_fold_probs, y_true_pool, y_prob_pool,
                    ...
                    f"ROC — {outcome} (Logistic Regression, nested CV)")
    ...
    plot_top_features(final_features_df["feature"].tolist(),
                      final_features_df["lr_importance"].tolist(),
                      plots_dir / "lr_feature_importance.png",
                      f"Top LR feature importance — {outcome}",
