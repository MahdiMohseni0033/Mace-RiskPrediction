...
only the classifier (a StandardScaler -> SVC(kernel="rbf") Pipeline) and
its grid differ. Reads the per-outcome ranking from
SVM/results/Within{k}Yr/final_selected_features.csv (sorted by
`svm_importance`, i.e. permutation importance).
"""
...
def load_ranked_features(outcome: str) -> pd.DataFrame:
    ...
    if not path.exists():
        raise FileNotFoundError(
            f"Cannot find '{path}'. Run SVM/src/train_svm.py first."
        )
...
def fit_one_k(X_k, y, outer, fixed_best_params):
    ...
    for fold_idx, (tr, te) in enumerate(outer.split(X_k, y)):
        ...
        if HYPERPARAM_STRATEGY == "per_fold_grid":
            ...
            base = make_svm_pipeline(random_state=fold_seed)
            gs = GridSearchCV(
                base, SVM_PARAM_GRID, scoring=INNER_SCORING, cv=inner,
            ...
            model = make_svm_pipeline(random_state=fold_seed)
...
def run_outcome(outcome: str, df: pd.DataFrame) -> dict:
    ...
    peak_summary = {
        ...
        "param_grid": SVM_PARAM_GRID if HYPERPARAM_STRATEGY == "per_fold_grid" else None,
...
def write_outcome_report(out_dir, outcome, metrics_df, peak, ranked,
    ...
    lines = [
        ...
        f"- Feature ranking source: `SVM/results/{outcome}/"
