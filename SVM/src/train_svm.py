"""Train one SVM (RBF kernel) classifier per outcome horizon.

Mirrors Random_Forest/src/train_rf.py: same nested CV, same stability-
selection feature step, same output schema. Only the classifier (a
StandardScaler -> SVC(kernel="rbf") Pipeline) is different.

Importance for the final model is permutation importance — RBF kernels
have no native feature_importances_.

Run:
    python SVM/src/train_svm.py
"""
...
def make_svm_pipeline(*, random_state: int) -> Pipeline:
    return Pipeline([
...
# --------------------------------------------------------------------------- #
def fit_one_outer_fold(X_train, y_train, X_test, y_test, feature_names, fold_seed):
    ...
    pipe = make_svm_pipeline(random_state=fold_seed)
    gs = GridSearchCV(
        pipe, SVM_PARAM_GRID, scoring="roc_auc", cv=inner,
    ...
    train_prob = best.predict_proba(Xtr_sel)[:, 1]
...
def train_outcome(df: pd.DataFrame, outcome: str, feature_names: list[str]) -> dict:
    ...
    # ----- final model: stability-select on all data, refit SVM with best CV params -----
    ...
    final_pipe = make_svm_pipeline(random_state=RANDOM_STATE)
    final_pipe.set_params(**most_common)
    X_sel_full = X[:, sel_idx_full]
    final_pipe.fit(X_sel_full, y)

    # Permutation importance on the full data — RBF SVM has no native importances.
    perm = permutation_importance(
        final_pipe, X_sel_full, y,
    ...
    final_features_df = pd.DataFrame({
        ...
        "svm_importance": perm_imp,
    }).sort_values("svm_importance", ascending=False)
    ...
    plot_roc_curves(per_fold_probs, y_true_pool, y_prob_pool,
                    ...
                    f"ROC — {outcome} (SVM-RBF, nested CV)")
    ...
    plot_top_features(final_features_df["feature"].tolist(),
                      final_features_df["svm_importance"].tolist(),
                      plots_dir / "svm_feature_importance.png",
                      f"Top SVM permutation importance — {outcome}",
                      "Mean drop in ROC AUC when permuted")
